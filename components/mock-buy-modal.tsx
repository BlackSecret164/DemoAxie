...
export function MockBuyModal({ isOpen, onClose, creature }: MockBuyModalProps) {
  const { wallet, isLoading, buyNFT, estimateGas } = useMockWeb3()
  ...
  const hasInsufficientFunds = wallet.isConnected ? wallet.balance < totalCost : false

  useEffect(() => {
    if (isOpen && creature) {
      estimateGas().then(setGasEstimate)
    }
  }, [isOpen, creature, estimateGas])
  ...
  const handleBuy = async () => {
    ...
    try {
      const transaction = await buyNFT(creature.id, creature.price)
      ...
      if (transaction) {
        ...
        purchaseNFT(creature.id, wallet.address)
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start space-x-2 bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
            ...
            <div className="text-sm text-blue-200">
              <p className="font-medium mb-1">Simulated Smart Contract Purchase</p>
              <p>
                This is a simulation of blockchain transaction. No real ETH will be spent.
              </p>
            </div>
          </div>
          ...
          <div className="flex space-x-3">
            ...
            <Button
              onClick={handleBuy}
              disabled={!wallet.isConnected || isProcessing || hasInsufficientFunds}
