...
import { MockSellModal } from "@/components/mock-sell-modal"
import { Swords, DollarSign, Crown, Zap, Shield, Sword } from "lucide-react"
...
interface MockMyAxieCardProps {
  axie: MyAxie
}

export function MockMyAxieCard({ axie }: MockMyAxieCardProps) {
  ...
  const getClassIcon = (className: string) => {
    ...
    return icons[className as keyof typeof icons] || <Crown className="w-3 h-3" />
  }

  // Mock experience and level
  const exp = Math.floor(Math.random() * 2000) + 500
  const level = Math.floor(exp / 100) + 1
  const expToNextLevel = 100 - (exp % 100)
        ...
        <CardContent className="p-4 relative z-10">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <Progress value={exp % 100} className="h-2 bg-white/10" />
            </div>
