...
interface SellModalEnhancedProps {
  isOpen: boolean
  onClose: () => void
  axie: any
  onConfirmSale: (axieId: number, price: string, txHash: string) => void
}

export function SellModalEnhanced({ isOpen, onClose, axie, onConfirmSale }: SellModalEnhancedProps) {
  const { isConnected, listAxie } = useWeb3()
  ...
  const handleSell = async () => {
    ...
    try {
      const txHash = await listAxie(axie.id, price)
      
      if (txHash) {
        onConfirmSale(axie.id, price, txHash)
        onClose()
        setPrice("")
      }
    } catch (error) {
      ...
      <DialogContent className="bg-gray-900 border-white/20 text-white max-w-md">
        ...
        <div className="space-y-6">
          ...
          <div className="flex space-x-3">
            ...
            <Button
              onClick={handleSell}
              disabled={!isConnected || !price || priceNumber <= 0 || isProcessing}
